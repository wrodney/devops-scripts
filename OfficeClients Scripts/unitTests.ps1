...
task Run.UnitTests -description "Run all unit tests using nUnit" {
    ...
    [string]$testProjectPath = [System.IO.Path]::Combine($src_directory, "*.Test.csproj")
    ...
    foreach ($testProjectFile in $testProjectFiles)
    {
        ...
        # C:\tmp\build\Diligent.Research.Test.xml
