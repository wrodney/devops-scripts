...
& "$this_directory\tools\nuget.exe" restore $src_directory\WindowsDesktop.sln
& "$this_directory\tools\nuget.exe" restore $src_directory\WindowsDesktop.sln -Source "http://artifactory-clt.devtms.local:8081/artifactory/api/nuget/research-nuget-local"
& "$this_directory\tools\nuget.exe" restore $src_directory\WindowsDesktop.sln -Source "http://artifactory-clt.devtms.local:8081/artifactory/api/nuget/teams-nuget-local"

# Compile
& "C:\windows\Microsoft.NET\Framework64\v4.0.30319\msbuild" $src_directory\WindowsDesktop.sln /p:Configuration=$buildConfig /p:Platform="Any CPU" | Out-Null

#
if ($LASTEXITCODE -gt 0)
{
     write-output "Failed: msbuild of WindowsDesktop.sln"
     exit $LASTEXITCODE 
}

if ($LASTEXITCODE -eq 0)
{
     write-output "Success: clean exit from msbuild of WindowsDesktop.sln"
} 

# Run unit tests
& "$this_directory\tools\nunit.2.6.4\nunit-console.exe" $src_directory\Desktop\ViewModelTests\Diligent.Teams.WindowsDesktop.ViewModel.Tests.csproj /xml:ViewModel.Tests.Result.xml /config:$buildConfig  
...
{
	& "$this_directory\tools\nunit.2.6.4\nunit-console.exe" $src_directory\Desktop\Tests\Diligent.Teams.WindowsDesktop.Tests.csproj /xml:Integration.Tests.Result.xml /config:$buildConfig  
}
...
& "C:\windows\Microsoft.NET\Framework64\v4.0.30319\msbuild" $src_directory\WindowsDesktop.sln /t:Publish /p:Configuration=$buildConfig /p:Platform="Any CPU" /p:BootstrapperEnabled=false | Out-Null
...
# Copy the the build files to http://clt01d1web14.devtms.local:58200/application/WindowsDesktop
...
. robocopy "$src_directory\Desktop\Application\bin\$buildConfig" "\\clt01d1web13.devtms.local\deploy\WindowsDesktop" *.* /e /np /r:0
